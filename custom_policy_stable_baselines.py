...
class CustomPolicy(DDPGPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None,
        ...
        self.RANDOM_SEED = int((time.time()%10)*1000)
        self.n_init = tf.keras.initializers.TruncatedNormal(seed=self.RANDOM_SEED)
        self.u_init = tf.keras.initializers.RandomUniform(minval=-3e-3, maxval=3e-3, seed=self.RANDOM_SEED)
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for i, layer_size in enumerate(self.layers):
                pi_h = tf.layers.dense(pi_h, layer_size, name='fc' + str(i),kernel_initializer=self.n_init)
                if self.layer_norm:
    ...
    def make_critic(self, obs=None, action=None, reuse=False, scope="qf"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            netx1 = tf.layers.dense(qf_h, layer_size_1, name='netx1',activation= 'relu',kernel_initializer=self.n_init)
            netu1 = tf.layers.dense(action, layer_size_2, name='netu1',kernel_initializer=self.n_init)
            netx2 = tf.layers.dense(netx1,layer_size_1,name='netx2',activation = 'relu',kernel_initializer=self.n_init)
            netu2 = tf.layers.dense(netu1,layer_size_2,name='netu2',activation = 'relu',kernel_initializer=self.n_init)
