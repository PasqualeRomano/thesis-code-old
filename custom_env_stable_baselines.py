...
class AcrobotPyB(gym.Env):
    ...
    def __init__(self):
        ...
        self.robot = Robot("double_pendulum.urdf")
        self.robot.actuated_index = [2]
        self.robot.time_step = .01
        self.robot.GUI_ENABLED = 0
        self.robot.setupSim()
        self.robot.SINCOS = 1
        ...
        high = np.array([1., 1., self.max_speed,1.,1.,self.max_speed], dtype=np.float32)
        self.action_space = spaces.Box(
            low=-self.max_torque,
            high=self.max_torque, shape=(1,),
            dtype=np.float32
    ...
    def step(self, u):
        ...
        reward = -np.linalg.norm(goal-np.array([self.robot.states[2][0],self.robot.states[2][1],self.robot.states[2][2]])) 
    ...
    def _get_obs(self):
        self.robot.observeState()
        return np.array([self.robot.states_sincos[1][0],self.robot.states_sincos[1][1],self.robot.states_dot[1][3],
                         self.robot.states_sincos[2][0],self.robot.states_sincos[2][1],self.robot.states_dot[2][3]])
