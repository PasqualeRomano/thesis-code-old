...
env                 = Robot("single_pendulum.urdf",[1],[1])       # Continuous pendulum
...
class PolicyNetwork:
    def __init__(self):
        ...
        policy          = tflearn.fully_connected(net, NU,  activation='tanh', weights_init=u_init)*2.0 #max torque DKW
    ...
    def setupOptim(self):
        ...
        optim           = tf.train.AdamOptimizer(POLICY_LEARNING_RATE).\
            apply_gradients(list(zip(grad,self.variables)))
...
def rendertrial(maxiter=NSTEPS,verbose=True):
    ...
    for i in range(maxiter):
        ...
        x=np.array([[env_rend.states_sincos[0][0],env_rend.states_sincos[0][1],
                   env_rend.states_dot[0][3]]])
        reward = (-np.square(env_rend.states[0][3]))*0.1-np.square(env_rend.states_dot[0][3]*0.01)
        #env.render()
        time.sleep(1e-2)
        rsum += reward
...
for episode in range(1,NEPISODES):
    ...
    for step in range(NSTEPS):
        ...
        x2 = np.array([env.states_sincos[0][0],env.states_sincos[0][1],
                   env.states_dot[0][3]])   
        r = (-np.square(env.states[0][3]))*0.1-np.square(env.states_dot[0][3]*0.01)
...
#Display and logging (not mandatory).
    ...
    h_qva.append(maxq)
    h_ste.append(step)
    if not (episode+1) % 10:     rendertrial(NSTEPS)
...
f.write("NEPISODES = "+str(NEPISODES)+", NSTEPS = "+str(NSTEPS)+", QVALUE_LEARNING_RATE = "+str(QVALUE_LEARNING_RATE)+", POLICY_LEARNING_RATE = "+str(POLICY_LEARNING_RATE)+", DECAY_RATE = "+str(DECAY_RATE)+", UPDATE_RATE = "+str(UPDATE_RATE)+", REPLAY_SIZE"+str(REPLAY_SIZE)+", BATCH_SIZE"+str(BATCH_SIZE)+", NH1 = "+str(NH1)+", NH2 = "+str(NH2))
f.close() 
